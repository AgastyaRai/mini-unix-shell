    ...
    void eval(const char **toks, bool bg) { // bg is true iff command ended with &
        ...
        if (strcmp(toks[0], "nuke") == 0) {
            ...
            for (int i = 1; toks[i] != NULL; i++) {
                ...
                if (toks[i][0] == '%') {
                    ...
                    if (jobNumber > 32 || !jobs[jobNumber - 1].running) {
                        char errorMessage[MAXLINE];
                    ...
                    if (!jobs[jobNumber - 1].running) { 
                        char errorMessage[MAXLINE];
                        int errorMessageLength = snprintf(errorMessage, sizeof(errorMessage), "ERROR: job %d is not running\n", jobNumber);
                        write(STDERR_FILENO, errorMessage, errorMessageLength);
                        fflush(stdout);
                        continue;
                    }
                    ...
                    // kill the job
                    kill(jobs[jobNumber - 1].pid, SIGKILL);
                ...
                for (int j = 0; j < 32; j++) {
                    if (jobs[j].running && jobs[j].pid == pid) {
                        matchFound = true;
                        kill(jobs[j].pid, SIGKILL);
                        printf("[%d] (%d)  killed  %s\n", jobs[j].jobNumber, jobs[j].pid, jobs[j].commandName);
                        fflush(stdout);
                        break;
                    }
                }
        ...
        if (strcmp(toks[0], "fg") == 0) {
            ...
            if (toks[1][0] == '%') {
                ...
                if (jobNumber > 32 || (!jobs[jobNumber - 1].running && !jobs[jobNumber - 1].stopped)) {
                    char errorMessage[MAXLINE];
                    int errorMessageLength = snprintf(errorMessage, sizeof(errorMessage), "ERROR: no job %d\n", jobNumber);
                    write(STDERR_FILENO, errorMessage, errorMessageLength);
                    fflush(stdout);
                ...
                if (jobs[jobNumber - 1].stopped) {

                    // send a continue signal
                    kill(jobs[jobNumber - 1].pid, SIGCONT);

                    // wait for the job to resume
                    while (jobs[jobNumber - 1].stopped) {
                        usleep(1000);
                    }
                ...
                tcsetpgrp(STDIN_FILENO, jobs[jobNumber - 1].pid);

                // wait for the job to finish
                while (jobs[jobNumber - 1].running) {
                    usleep(1000);
                }
        ...
        if (processCount >= 32) {
            const char *msg = "ERROR: too many processes\n";
            write(STDERR_FILENO, msg, strlen(msg));
            return;
        }
        ...
        while (true) {
            ...
            for (int i = 0; i < 32; i++) {
                if (jobs[i].pid == pid) {
                    ...
                    // check if the child was suspended
                    ...
                    // free the memory for the command name
                    break;
                }
            }
